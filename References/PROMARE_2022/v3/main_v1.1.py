...
StateColors = {
    ...
    State.start: {"this": "yellow", "next": "green", "obj": ["yellow", "black"]},
    State.hole: {"this": "green", "next": "white", "obj": ""},
    State.mine: {"this": ["white_mine_back", "white_mine"], "next": "blue_mine", "obj": "black"},
    State.obstacle: {"this": "white2", "next": "", "obj": ""},
    State.door: {"this": "white_door", "next": "green", "obj": "blue_door"},
    State.bridge: {"this": "green", "next": "", "obj": ""},
    State.ball: {"this": "red", "next": "blue", "obj": "white"},
    State.stair: {
        "this": "green",
        "next": "white_stair",
        "obj": ["blue_stair", "green", "red"],
    },
    State.end: {"this": "yellow", "next": "green", "obj": ["yellow", "black"]},
}
# LAB色彩范围
color_range = {
    # 使用中
    ...
    "white_door": [(150, 0, 0), (255, 255, 255)],
    "white_mine_back": [(140, 0, 0), (255, 255, 255)],
    "white_mine": [(100, 0, 0), (255, 255, 255)],
    "white_stair": [(193, 0, 0), (255, 255, 255)],
    "white2": [(193, 0, 0), (255, 255, 255)],
    "black": [(0, 0, 0), (80, 255, 255)],
    "red": [(0, 156, 88), (255, 255, 255)],
    "blue_stair": [(35, 0, 0), (255, 130, 120)],
    "blue": [(49, 0, 0), (255, 255, 120)],
    "blue_door": [(20, 0, 0), (255, 255, 112)],
    "blue_mine": [(39, 100, 0), (255, 255, 100)],
...
def color_detect_sill(color, area_min=1000, grad=1.0, ksizes=None, defaults=None):
    """识别门槛
...
def color_detect_door(color_back, color_obj, grad=1.0, ksizes=None, defaults=None):
    ...
    if cnt_max is None:
        ...
        if len(cnts_roi) < 2:
            ...
            cnts_fl_area = [c for c in cnts_roi if cv2.contourArea(c) > 1000]
            cnts_fl_y = [
                c
...
def color_detect_bar(color1, color2, ksizes=None, has_green=True):
    if ksizes is None:
        ksizes_ = (3, 3)
    else:
        ksizes_ = ksizes
    ...
    img_bin_yellow = get_frame_bin(img, color1, ksizes_[0], ksizes_[1])
    cnts_yellow, _ = cv2.findContours(
        img_bin_yellow, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
    if cnts_yellow:
        cnts_area_yellow = [cv2.contourArea(c) for c in cnts_yellow]
        area_yellow = round(100 * sum(cnts_area_yellow) / area_full)
    else:
        cnts_yellow = [np.array([[[0, 0]], [[639, 0]], [[639, 479]], [[0, 479]]])]
        area_yellow = 0

    if has_green:
        img_bin_green = get_frame_bin(img, color2, ksizes_[0], ksizes_[1])
        cnts_green, _ = cv2.findContours(
            img_bin_green, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
        )
        if cnts_green:
            cnts_area_green = [cv2.contourArea(c) for c in cnts_green]
            area_green = round(100 * sum(cnts_area_green) / area_full)
        else:
            cnts_green = [np.array([[[0, 0]], [[639, 0]], [[639, 479]], [[0, 479]]])]
            area_green = 0
    else:
        cnts_green = [np.array([[[0, 0]], [[639, 0]], [[639, 479]], [[0, 479]]])]
        area_green = 0
    area_sum = area_yellow - area_green

    if frame_show:
        img_show = img.copy()
        put_text(img_show, f"area_sum:{area_sum:.2f}%", (50, 50))
        put_text(img_show, f"area_yellow:{area_yellow:.2f}%", (50, 100))
        put_text(img_show, f"area_green:{area_green:.2f}%", (50, 150))
        cv2.drawContours(img_show, cnts_yellow, -1, (0, 255, 255), 2)
        cv2.drawContours(img_show, cnts_green, -1, (0, 255, 0), 2)
        frames.put(img_show)

    return area_sum, area_yellow, area_green


def color_detect_ball(color, ksizes=None, defaults=None):
    if ksizes is None:
        ksizes_ = (3, 3)
    else:
        ksizes_ = ksizes
    if defaults is None:
        defaults_ = ((320, 240), (20, 20), 0)
    else:
        defaults_ = defaults
    ...
    if cnts:
        ellipses = [cv2.fitEllipse(c) for c in cnts if 200 < cv2.contourArea(c) < 4000]
        ellipses_fl = [e for e in ellipses if e[1][1] / e[1][0] < 1.5]
        if ellipses_fl:
            ball = ellipses_fl[0]
        else:
            ball = defaults_
    ...
    if frame_show:
        img_show = img.copy()
        cv2.ellipse(img_show, ball, (0, 255, 255), 2)
        cv2.circle(img_show, (int(ball_x), int(ball_y)), 2, (255, 0, 0), 2)
        put_text(img_show, f"ball:({ball_x:.0f},{ball_y:.0f})", (50, 50))
        frames.put(img_show)
...
def state_start(colors):
    print("state:start:enter")
    global state
    ksizes = [11, 7]
    has_green = True
    close_flag = False
    ...
    while state == State.start:
        time.sleep(0.5)
        area, _, _ = color_detect_bar(colors["this"], colors["next"], ksizes, has_green)
        print(f"state:start:area:{area}")
        if area > 5:
            print("state:start:wait")
            close_flag = True

        if close_flag and area < 5:
            AGC.runAction(Actions["go_LL"], 2, action_path)
            state = StateTrans[State.start]
            print("state:start:exit")
            break
    ...
    while state == State.mine:
        if step == -1:
            ...
            while step == -1:
                ...
                # )
                angle, points = color_detect_sill(
                    colors["next"], 400, grad, ksizes, defaults
                )
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                # 获取数据
                if camera_flag == 0:
                    _, angle, points = color_detect_top(colors["this"][0], grad, ksizes, defaults)
        ...
        if step == 2:
            ...
            while step == 2:
                ...
                time.sleep(0.5)
                angle, points = color_detect_sill(
                    colors["next"], area_min, grad, ksizes, defaults
                )
                _, center_y = points[0]
...
def state_door(colors):
    ...
    while state == State.door:
        if step == -1:
            ...
            init_camera_platform(870, 390)
            while step == -1:
                # _, angle, points = color_detect_top(
                #     colors["this"], grad, ksizes, defaults
                # )
                angles, points = color_detect_door(
                ...
                print(f"top: {top_angle:.2f}  ({top_center_x},{top_center_y})")
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                # 调整角度
                if not top_angle_range[0] < top_angle < top_angle_range[1]:
                    adjust_angle(top_angle, top_angle_range)
                    time.sleep(0.1)
                    AGC.runAction(Actions["squat"], 1, action_path)
                    time.sleep(0.5)
                    continue
                ...
                print("state:door:step 0 done, goto step 1")
                step = 1
                break
        ...
        if step == 1:
            ...
            while step == 1:
                ...
                # 左移
                AGC.runAction(Actions["left_go_L"], 9, action_path)
                # AGC.runAction(Actions["left_go_m"], 1, action_path)

                # 左转
                AGC.runAction(Actions["stand"], 6, action_path)
                AGC.runAction(Actions["turn_left_L1"], 7, action_path)
                # AGC.runAction(Actions["right_go_L"], 2, action_path)
...
def state_ball(colors):
    print("state:ball:enter")
    global state
    step = 0
    while state == State.ball:
        if step == -1:
            ksizes = [15, 15]
            grad = 2.5
            defaults = ((320, 240), (20, 20), 0)
            init_action()
            init_camera_platform(500, 450)
            while step == -1:
                time.sleep(0.5)
                ...
                # color_detect_ball(colors["obj"], ksizes, defaults)

        if step == 0:
            ksizes = [15, 11]
            grad = 2.5
            defaults = [0, (320, 385), (300, 385), (340, 385)]
            angle_range = [-6, 4]
            center_x_range = [300, 340]
            center_y_LL = 120
            center_y_L = 240
            ...
            while step == 0:
                ...
                if center_y < center_y_LL:
                    AGC.runAction(Actions["go_LL"], 1, action_path)
                    continue

                if center_y < center_y_L:
                    AGC.runAction(Actions["go_L"], 1, action_path)
                    continue
...
def state_stair(colors):
    ...
    while state == State.stair:
        ...
        if step == -2:
            ...
            angle_range = [-3, 3]
            center_x_range = [260, 400]
            center_y_switch = 100
            center_y_switch_L = 180
            center_y_stop = 265
            init_action()
            init_camera_platform(500, 450)
            while step == -2:
        ...
        if step == 4:
            ...
            while step == 4:
                # 判断位置, 是否进入下一步
                if stair < 0:
                    print("state:stair:step 4 done, goto step 5")
                    AGC.runAction(Actions["right_go_m"], 1, action_path)
                    step = 5
                    break
        ...
        if step == 5:
            for _ in range(5):
                AGC.runAction(Actions["go_L"], 1, action_path)
                time.sleep(0.2)
            AGC.runAction(Actions["turn_right_m"], 2, action_path)
            time.sleep(0.1)
            # AGC.runAction(Actions["right_go_L"], 1, action_path)
            time.sleep(0.1)
            # AGC.runAction(Actions["right_go_m"], 1, action_path)
        ...
        if step == 6:
            ...
            while step == 6:
                ...
                AGC.runAction(Actions["go_L"], 1, action_path)
                step = 9
                state = StateTrans[State.stair]
                break


def state_end(colors):
    print("state:end:enter")
    global state
    ksizes = [11, 7]
    has_green = False
    close_flag = False
    ...
    while state == State.end:
        time.sleep(0.5)
        area, _, _ = color_detect_bar(colors["this"], colors["next"], ksizes, has_green)
        print(f"state:end:area:{area}")
        if area > 5:
            print("state:end:wait")
            close_flag = True

        if close_flag and area < 5:
            AGC.runAction(Actions["go_LL"], 2, action_path)
            state = StateTrans[State.end]
            print("state:end:exit")
            break
