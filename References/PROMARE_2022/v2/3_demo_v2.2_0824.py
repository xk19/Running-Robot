...
StateColors = {
    ...
    State.mine: {"this": ["white2", "white_mine"], "next": "blue", "obj": "black"},
    State.obstacle: {"this": "white2", "next": "", "obj": ""},
    State.door: {"this": "white", "next": "green", "obj": "blue2"},
    State.bridge: {"this": "green", "next": "", "obj": ""},
    State.ball: {"this": "red", "next": "blue", "obj": "white"},
    State.stair: {"this": "blue", "next": "white2", "obj": ["blue", "green", "red"]},
...
def color_detect_door(color_back, color_obj, grad=1.0, ksizes=None, defaults=None):
    ...
    if cnt_max is None:
        ...
        if len(cnts_roi) < 2:
            ...
            cnts_fl_y = [c for c in cnts_roi if cv2.boundingRect(c)[1] + cv2.boundingRect(c)[3] < 360]
            if len(cnts_fl_y) < 2:
                cnts_max_two = [np.array([[[0, 0]], [[639, 0]], [[639, 479]], [[0, 479]]])]
                angle, center, left, right = defaults_
            else:
...
def color_detect_bar(color1, color2, ksizes=None, has_green=True):
    ...
    cnts_yellow, _ = cv2.findContours(img_bin_yellow, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    if cnts_yellow:
    ...
    if has_green:
        ...
        cnts_green, _ = cv2.findContours(img_bin_green, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        if cnts_green:
    ...
    while state == State.stair:
        ...
        if step == 6:
            ...
            while step == 6:
                # 获取数据
                time.sleep(0.5)
                _, angle, points = color_detect_top(colors["next"], grad, ksizes, defaults)
                center_x, _ = points[0]
