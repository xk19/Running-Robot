...
def color_detect_ellipse_near(
    color, ksizes, area_min=1000, area_max=3500, defaults=((300, 0), (10, 10), 0)
):
    # 图像处理及roi提取
    global frame_cali
    img = frame_cali.copy()
...
def state_mine(colors):
    ...
    while state == State.StateMine:
        ...
        if step == 1:
            ...
            if 120 < elps_x < 530:
                if left_flag == 0:
                    AGC.runAction(Actions["left_go_s"], 3, action_path)
                    continue
                else:
                    AGC.runAction(Actions["right_go_s"], 3, action_path)
                    continue
        ...
        if step == 2:
            ...
            if points[0][1] < 480:
                # 进入下一步
                print("state:mine:step 2 done, goto step 1")
                step = 1
                continue
