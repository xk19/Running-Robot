...
# 动作组路径及常用动作
action_path = "/home/pi/AiNexPro/AiNexPro/wsh/"
Actions = {
    "stand": "stand",
    "go_s": "little_goN",
    "go_L": "standgoN",
    "turn_left": "turn_leftN",
    "turn_right": "turn_rightN0",
    "left_go_s": "left_goN",
    "left_go_L": "left_goNe2",
    "right_go_s": "right_goN",
    "right_go_L": "right_go_L"
}
...
# 关卡状态处理函数
# 通过 globals().get(StateCalls[state])(colors) 实现调用
StateCalls = {
...
def adjust_angle(angle, angle_range):
    ...
    if angle < angle_range[0]:
        ...
        AGC.runAction(Actions["turn_right"], 1, action_path)
        time.sleep(0.5)
    elif angle > angle_range[1]:
        print("adjust:angle =", angle, "turn_left")
        AGC.runAction(Actions["turn_left"], 1, action_path)
        time.sleep(0.5)
    else:
...
def state_debug(colors):
    ...
    global state, frame, frame_cali, frame_flag, frame_show
    frame_show = True
    print("state:debug:enter")
    while state == State.StateDebug:
        color_detcet_top(colors["this"], grad=2.5, smooth_size=11, kernerl_size=7)


def state_hole_old(color_this="green"):
    """state:hole

    Args:
        color_this (str, optional): 关卡颜色. Defaults to "green".
    """
    global state, frame, frame_cali, frame_flag
    if state != State.StateHole:
        return
    ...
    while state == State.StateHole:
        if step == 1:
            _, top_angle, _, top_left, _ = color_detcet_top(color_this, grad=2.5, smooth_size=11, kernerl_size=7)
            ...
            if not left_range[0] < top_left[0] < left_range[1]:
                if left_flag == 0:
                    AGC.runAction(Actions["left_go_L"], 10, action_path)
                    time.sleep(0.2)
                    left_flag = 1
                    continue
                if left_flag == 1:
...
def state_bridge(color_this="green"):
    pass
...
if __name__ == "__main__":
    ...
    frame_show = True
    state = State.StateHole
    init_action()
    init_camera_platform(500, 300)
    while True:
        globals().get(StateCalls[state], lambda x:print("State not found"))(StateColors[state])