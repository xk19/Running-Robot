...
# 关卡中用到的颜色
StateColors = {
    State.debug: {"this": "green", "next": "", "obj": ["white", "black", "blue"]},
    State.hole: {"this": "green", "next": "white", "obj": ""},
    State.bridge: {"this": "green", "next": "", "obj": ""},
    State.mine: {"this": "white", "next": "blue", "obj": "black"},
    State.obstacle: {"this": "white2", "next": "", "obj": ""},
    State.door: {"this": "red", "next": "green", "obj": "blue"},
...
def color_detect_top(color, grad=1.0, ksizes=None, defaults=None):
    ...
    if frame_show:
        ...
        cv2.circle(img_show, tuple(top_center), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(top_left), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(top_right), 5, (0, 255, 255), 2)

        # 显示图像
        image = np.hstack((cv2.cvtColor(close, cv2.COLOR_GRAY2BGR), img_show))
...
def color_detect_bottom(color, grad=1.0, ksizes=None, defaults=None):
    ...
    if frame_show:
        ...
        cv2.circle(img_show, tuple(bottom_center), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(bottom_left), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(bottom_right), 5, (0, 255, 255), 2)
...
def color_detect_mine_near(color_back, color_obj, ksizes=None, area=None, default=None):
    ...
    if cnts:
        ...
        if mines:
            mine_near = min(mines, key=lambda m: (1.2*(m[0]-320))**2 + (m[1]-480)**2)
        else:
            mine_near = default_
    else:
        mine_near = default_
    ...
    if frame_show:
        ...
        # 显示图像
        image = np.hstack((cv2.cvtColor(img_mine_bin, cv2.COLOR_GRAY2BGR), img_show))
        cv2.imshow("image", image)
        cv2.waitKey(1)
...
def color_detect_sill_bottom(color, grad=1.0, ksizes=None, defaults=None):
    ...
    Returns:
        tuple: (bottom_angle, [bottom_center, bottom_left, bottom_right])
    """
    if ksizes is None:
        ksizes_ = (3, 3)
    else:
    ...
    if frame_show:
        ...
        # 显示图像
        image = np.hstack((cv2.cvtColor(img_bin, cv2.COLOR_GRAY2BGR), img_show))
        cv2.imshow("image", image)
        cv2.waitKey(1)
...
def color_detect_door(color_back, color_obj, grad=1.0, ksizes=None, defaults=None):
    ...
    if frame_show:
        ...
        cv2.circle(img_show, tuple(center), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(left), 5, (0, 255, 255), 2)
        cv2.circle(img_show, tuple(right), 5, (0, 255, 255), 2)
...
def state_debug(colors):
    ...
    while state == State.debug:
        color_detect_top(colors["this"], grad, ksizes[0], defaults)
    ...
    while state == State.mine:
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                # 低头前，判断是否低头
                if head_flag == 0 and center_y > center_y_switch:
                    head_flag = 1
                    continue

                # 低头后，判断是否停止
                if head_flag == 1 and center_y > center_y_stop:
                    # 进入下一关
        ...
        if step == 1:
            ...
            mine_y_little = 260
            mine_y_stop = 360
            mine_x_range = [120, 485]
            big_flag = 1
            m_x_b_range = [-100, 100]

            # 低头看地雷
            init_camera_platform(500, 360)
            time.sleep(0.25)
            while step == 1:
                time.sleep(0.25)
                time_sleep += 0.45
                print(f"state:mine:sleep: {time_sleep:.2f}")
                print(f"state:mine:total: {time.time() - start:.2f}")
                _, _, points = color_detect_top(
                    colors["this"], grad, ksizes[0], defaults
                )
                _, center_y = points[0]
                if center_y > 360:
                    state = StateTrans[State.mine]
...
def state_stair(colors):
    ...
    while state == State.stair:
        if step == -1:
            ...
            while step == -1:
                _, angle, points = color_detect_top(
                    colors["obj"][1], grad, ksizes, defaults
                )
...
if __name__ == "__main__":
    ...
    # 开启采集子进程
    cap_proc = mp.Process(target=capture, args=(cap_stack, 10), daemon=True)
    cap_proc.start()

    try:
        # 关卡处理
        print("process:main:running")
        err_func = lambda x: print(f'funciton "{StateCalls[state]}" not defined.')
        while state != State.idle:        
            globals().get(StateCalls[state], err_func)(StateColors[state])
    
    finally:
        # 关闭子进程
        print("process:main:exit")
        cap_proc.terminate()
        cap_proc.join()
