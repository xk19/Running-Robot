...
StateTrans = {
    ...
    State.StateObstacle: State.StateDebug,
}
...
def color_detect_top(
    color, grad=1.0, ksizes=[3, 3], defaults=[0, [320, 240], [300, 240], [340, 240]]
):
...
def color_detect_ellipse_near(
    color,
    ksizes=[3, 3],
    area_min=500,
    area_max=3000,
    defaults=((300, 0), (10, 10), 0),
):
    # roi提取
    global frame_cali
    img = frame_cali.copy()
    ...
    img_transform = cv2.cvtColor(img_smooth, cv2.COLOR_BGR2LAB)
    img_thresh = cv2.inRange(
        img_transform, color_range["white"][0], color_range["white"][1]
    )
    kernel = np.ones((ksizes[1], ksizes[1]), np.uint8)
    open = cv2.morphologyEx(img_thresh, cv2.MORPH_OPEN, kernel)
    close = cv2.morphologyEx(open, cv2.MORPH_CLOSE, kernel)
    ...
    # 图像处理
    img_smooth = cv2.GaussianBlur(img_roi, (ksizes[0], ksizes[0]), 0)
    img_transform = cv2.cvtColor(img_smooth, cv2.COLOR_BGR2LAB)
    img_thresh = cv2.inRange(
        img_transform, color_range[color][0], color_range[color][1]
    )
    ...
    if cnts is None:
        ...
        for cnt in cnts:
            ...
            if (area_min < cv2.contourArea(cnt) < area_max):
                # print(f"area={cv2.contourArea(cnt):.2f}")
                ellipses.append(ellipse)

    if len(ellipses) == 0:
        ellipse_near = defaults
    else:
        ellipse_near = max(ellipses, key=lambda x: x[0][1])

    if frame_show:
        vis_img = img.copy()
        cv2.putText(
            vis_img,
            f"ellipse near (x,y): ({ellipse_near[0][0]:.0f},{ellipse_near[0][1]:.0f})",
            (50, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (255, 255, 255),
        ...
        cv2.ellipse(vis_img, ellipse_near, (255, 255, 0), 2)
        cv2.imshow("vis_img", vis_img)
        cv2.imshow("close", close)
        cv2.waitKey(1)

    return int(ellipse_near[0][0]), int(ellipse_near[0][1])
    ...
    while state == State.StateMine:
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                if head_flag == 0 and top_center[1] > y_switch:
                    head_flag = 1
                    continue

                if head_flag == 1 and top_center[1] > y_stop:
                    # 进入下一关
        ...
        if step == 1:
            ...
            while step == 1:
                ...
                if mine_x_range[0] < elps_x < mine_x_range[1]:
                    if elps_x > 300:
    ...
    if step == 0:
        ...
        while step == 0:
            ...
            if not center_x_range[0] < top_center[0] < center_x_range[1]:
                if top_center[0] > center_x_range[1]:
                    AGC.runAction(Actions["right_go_s"], 1, action_path)
                    continue
                if top_center[0] < center_x_range[0]:
                    AGC.runAction(Actions["left_go_s"], 1, action_path)
                    continue
