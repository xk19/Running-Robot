...
StateColors = {
    ...
    State.hole: {"this": "green", "next": "", "obj": ""},
    State.mine: {
        "this": ["white_road", "white_mine"],
        "next": "blue_mine",
        "obj": "black",
    },
    State.obstacle: {"this": "", "next": "", "obj": ""},
    State.door: {"this": "white_door", "next": "green", "obj": "blue_door"},
    State.bridge: {"this": "green", "next": "", "obj": ""},
    State.ball: {"this": "red", "next": "", "obj": "white_ball"},
    State.stair: {
        "this": "green",
        "next": "white_stair",
...
def state_debug(colors):
    ...
    while state == State.debug:
        color_detect_top(color, grad, ksizes)
        press_key = cv2.waitKey(10) & 0xFF
        if press_key == ord("b"):
            print("black")
            color = "black"
    ...
    while state == State.mine:
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                if camera_flag == 0:
                    _, angle, points = color_detect_top(
                        colors["this"][0], grad, ksizes, defaults
                    )
...
def state_obstacle(colors):
    ...
    while state == State.obstacle:
        ...
        for _ in range(4):
            AGC.runAction(Actions["back_L"], 1, action_path)
            time.sleep(0.5)
    ...
    while state == State.door:
        ...
        if step == 1:
            ...
            while step == 1:
                ...
                # 左转
                AGC.runAction(Actions["stand"], 1, action_path)
    ...
    while state == State.ball:
        ...
        if step == 0:
            ...
            while step == 0:
                ...
                # if right_x > right_x_range[1]:
                #     AGC.runAction(Actions["right_go_m"], 1, action_path)
                #     continue
                if right_x< right_x_range[0]:
                    AGC.runAction(Actions["left_go_m"], 1, action_path)
                    continue

                # if not right_x_range[0] < right_x < right_x_range[1]:
                #     adjust_x(right_x, right_x_range)
                #     continue
        ...
        if step == 2:
            AGC.runAction(Actions["go_L"], 1, action_path)
            AGC.runAction(Actions["turn_left_L"], 4, action_path)
